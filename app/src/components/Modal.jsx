    ...
    <div className="modal show" style={{ display: 'grid' }}>
      ...
      <div className="modalBox" style={{ zIndex: 10 }}>
        ...
        <div className="acts" style={{ justifyContent: onDelete ? 'space-between' : 'flex-end' }}>
          ...
          {onDelete && (
            <button className="btn-modal btn-delete" onClick={onDelete}>Supprimer</button>
          )}

          {/* Groupe Annuler/Enregistrer à droite */}
          <div style={{ display: 'flex', gap: 10 }}>
            <button className="btn-modal btn-cancel" onClick={onClose}>Annuler</button>
            <button className="btn-modal btn-save" onClick={onSave}>Enregistrer</button>
          </div>
        </div>
      </div>
    </div>
