...
const FormVendor = ({ onChange, initialData }) => {
  const [formData, setFormData] = useState({
    name: '', category: APP_CONFIG.categories.vendors[0], price: 0, status: 'contact', email: ''
  });
  ...
  return (
    <>
      <div className="field"><label>Nom du prestataire</label>
      ...
      <div className="field"><label>Catégorie</label>
        <select value={formData.category} onChange={(e) => handleChange('category', e.target.value)}>
          {APP_CONFIG.categories.vendors.map(cat => <option key={cat} value={cat}>{cat}</option>)}
        </select>
      </div>
      ...
      <div className="field"><label>Statut</label>
        <select value={formData.status} onChange={(e) => handleChange('status', e.target.value)}>
           {Object.entries(APP_CONFIG.statuses.vendors).map(([k, l]) => <option key={k} value={k}>{l}</option>)}
        </select>
      </div>
