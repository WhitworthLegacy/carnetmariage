...
function App() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (localId) {
        ...
        // --- CREATE (Création) ---
        const { data, error } = await supabase.from(currentConfig.table).insert([payload]).select();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      currentConfig.setter(
        currentConfig.list.filter((item) => item[currentConfig.idField] !== localId)
      );
    ...
    setWidgets(
      widgets.map((w) => {
        if (w.id === id) {
          ...
          if (w.size === "col-4")
            newSize = "col-6"; // 33%  -> 50%
          else if (w.size === "col-6")
            newSize = "col-8"; // 50%  -> 66%
          else if (w.size === "col-8")
            newSize = "col-12"; // 66%  -> 100%
  ...
  return (
    <div className="app-container">
        ...
        {!loading &&
          widgets.map((widget) => (
            ...
            >
              ...
              {widget.id === "tasks" && (
                <WidgetTasks tasks={tasks} onTaskClick={(t) => openModal("tasks", t)} />
              )}
              {widget.id === "budget" && (
                <WidgetBudget budgetLines={budget} onLineClick={(b) => openModal("budget", b)} />
              )}
              {widget.id === "vendors" && (
                <WidgetVendors vendors={vendors} onVendorClick={(v) => openModal("vendors", v)} />
              )}
              {widget.id === "venues" && (
                <WidgetVenues venues={venues} onVenueClick={(l) => openModal("venues", l)} />
              )}
              {widget.id === "guests" && (
                <WidgetGuests guests={guests} onGuestClick={(g) => openModal("guests", g)} />
              )}
            </Card>
          ))}
      </div>
      ...
      <Modal
        isOpen={isModalOpen}
        title={
          tempData.id || tempData.taskId || tempData.lineId || tempData.guestId
            ? "Modifier"
            : "Ajouter"
        }
      ...
      >
        ...
        {modalType === "tasks" && (
          <FormTask
            onChange={(d) => setTempData({ ...tempData, ...d })}
            initialData={tempData.taskId ? tempData : null}
          />
        )}
        {modalType === "budget" && (
          <FormBudget
            onChange={(d) => setTempData({ ...tempData, ...d })}
            initialData={tempData.lineId ? tempData : null}
          />
        )}
        {modalType === "vendors" && (
          <FormVendor
            onChange={(d) => setTempData({ ...tempData, ...d })}
            initialData={tempData.id ? tempData : null}
          />
        )}
        {modalType === "venues" && (
          <FormVenue
        ...
        {modalType === "guests" && (
          <FormGuest
            onChange={(d) => setTempData({ ...tempData, ...d })}
            initialData={tempData.guestId ? tempData : null}
          />
        )}
      </Modal>
    </div>
