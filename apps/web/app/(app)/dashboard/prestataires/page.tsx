...
export default function PrestatairesPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingId) {
        setVendors((prev) => prev.map((v) => (v.id === editingId ? json.data : v)));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="font-serif text-2xl sm:text-3xl text-ink">Prestataires</h1>
          ...
          loading ? (
            ...
            <div className="overflow-x-auto pb-4 -mx-2">
              <div className="flex gap-3 min-w-max px-2">
                {(Object.keys(VENDOR_STATUSES) as VendorStatus[]).map((statusKey) => (
                  <div key={statusKey} className="w-48 flex-shrink-0 bg-ivory rounded-2xl p-3">
                    ...
                    <div className="space-y-2">
                      {(groupedByStatus[statusKey] ?? []).map((vendor) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-1">
                            <div className="flex-1 min-w-0" onClick={() => openEditModal(vendor)}>
                              <p className="text-sm font-medium text-ink truncate">{vendor.name}</p>
                              <Badge variant="purple" className="text-[10px] mt-1.5">
                                {vendor.category}
                              </Badge>
                              {vendor.price > 0 && (
                      ...
                      {(groupedByStatus[statusKey] ?? []).length === 0 && (
                        <p className="text-[10px] text-muted text-center py-6">Aucun</p>
                      )}
                    </div>
                  </div>
                ))}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-brand-border">
                      ...
                      <th className="text-left font-medium text-muted px-4 py-3">Prix</th>
                      <th className="text-left font-medium text-muted px-4 py-3">Statut</th>
                      <th className="text-left font-medium text-muted px-4 py-3">Email</th>
                    ...
                    {vendors.map((vendor) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-ink">
                          {vendor.price > 0 ? formatPrice(vendor.price) : "—"}
                        </td>
                        <td className="px-4 py-3">
