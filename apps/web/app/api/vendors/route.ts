...
import { CreateVendorSchema, makeError } from "@carnetmariage/core";
...
export async function POST(request: NextRequest) {
  ...
  const { allowed, current, limit } = await checkPlanLimit(
    auth.supabase, auth.wedding!.id, auth.wedding!.plan, "vendors"
  );
  if (!allowed) {
    return jsonError(makeError("PLAN_LIMIT_REACHED", `Limite atteinte : ${current}/${limit} prestataires. Passe à Premium pour en ajouter plus.`), 403);
  }
